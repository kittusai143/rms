...
const AdminNotification = (props) => {
  ...
  const status = useSelector((state) => state?.login?.tasks)
  ...
  const handleAccept = (notificationData, item, on) => {
    ...
    if (hasRequestedSoftBlock >= 2) {
      ...
      else if (item.ProcessStatus === "Allocation Extension Requested") {
        newStatus = on ? "Allocated" : null;
      }
        ...
        putReq(`${url}ResourceAllocProcess/update/${item.Id}`, {
          ...
          if (response.data) {
            ...
            if (newStatus === "SoftBlocked") {
              ...
              props.setSToast(`Resource  Allocation approved successfully`);
            }
      ...
      else
      {
        ...
        putReq(`${url}ResourceAllocProcess/update/${item.Id}`, {
          ...
          if(response.data)
          {
          props.setSToast(`Resource Allocation extension request rejected successfully`);
          }
          else
          {
          props.setEToast(`Server failure`);
          }
      })
      props.onClose();
    }
    ...
    try {
      ...
      if (response && response.data) {
        const filteredAllocationData = response.data.filter(item =>
          ...
          item.ProcessStatus === "Allocation Extension Requested" ||
          item.ProcessStatus === "Allocation Rejected"
        );
        // console.log(filteredAllocationData);
        if (filteredAllocationData.length > 0) {
          // console.log("hiiiiii", filteredAllocationData)
            ...
            <Tab eventKey="All" title="All">
              {notificationData.map((item, index) => (
                <Container
                  ...
                  fluid
                  // className="m-1"
                  className={item.PmoReadStatus ? "bg-white" : "bg-secondary bg-opacity-10"}
                  style={{
                ...
                >
                    ...
                    <div>
                      <Row className="notify " >
                        ...
                        <Col
                          ...
                          display="flex"
                          style={{
                            fontWeight: (item.PmoReadStatus) ? 'none' : 'bold'
                          }}
                        >
                          {
                            (() => {
                              ...
                              switch (item.ProcessStatus) {
                                ...
                                case 'De-Allocation Requested':
                                  ...
                                  break;
                                case 'Allocation Extension Requested':
                                  message = `${item.createdByName} has requested Allocation Extension for ${item.ResourceName}`;
                                  break;
                                default:
                                  message = '';
                                  break;
                          ...
                          <div className="d-flex gap-1">
                            ...
                            <Col>
                              <Button
                                className="crossBtn"
                                onClick={() => {
                                  if(item.ProcessStatus !== "Allocation Extension Requested")
                                  {
                                   setreject(true);
                  ...
                  {info && clickedinfoItemIndex === index && (
                    <div style={{ padding: "2%" }}>
                      {/* {console.log(notificationData[index].ProcessStatus, index)} */}
                      {notificationData[index].ProcessStatus ===
                        "SoftBlock Requested" && (
                          <div>
                            <p>
                              {notificationData[index].ProcessStatus} <span>from</span>{" "}
                      ...
                      {(notificationData[index].ProcessStatus ===
                        "Allocation Extension Requested") && (
                          <div>
                            <p>
                              <b>Client Name:</b>{" "}
                              {notificationData[index]?.ClientName}
                            ...
                            <p>
                              <b>Extended Date:</b>{" "}
                              {new Date(notificationData[index]?.extendedDate)?.toLocaleDateString('en-GB').replace(/\//g, '-')}
                            </p>
                          </div>
                        )}
                      {/* {console.log(notificationData[index].ProcessStatus, index)} */}
                      ...
                      {(
                        ...
                        "De-Allocation Rejected") && (
                          <div>
                            ...
                            <p>
                              <b>Project Dates:</b>{" "}
                              {notificationData[index].allocStartDate?.slice(0, 10)}{" "}
                              <b>to</b>{" "}
                              {(notificationData[index]?.allocEndDate?.slice(0, 10))}
                            </p>
                            <p>
                              <b>Billability:</b>{" "}
                              {notificationData[index]?.Billability}
            ...
            <Tab eventKey="soft-block" title="Soft Block">
              {softBlockNoification.map((item, index) => (
                ...
                >
                  ...
                  {info && clickedinfoItemIndex === index && (
                    <div style={{ padding: "2%" }}>
                      {/* {console.log('------------', softBlockNoification[index].ProcessStatus, index)} */}
                ...
                >
                  <div className="d-flex justify-content-between">
                    ...
                    <div>
                      {
                        (select[0]?.empRoleName === "PMO Analyst" &&
                          item.ProcessStatus === "Allocation Requested") || (select[0]?.empRoleName === "PMO Analyst" &&
                            item.ProcessStatus === "Allocation Extension Requested") ? (
                          <div className="d-flex gap-1">
                            <Col>
                              <Button
                  ...
                  {info && clickedinfoItemIndex === index && (
                    <div style={{ padding: "2%" }}>
                      {/* {console.log(allocationNotification[index].ProcessStatus, index)} */}
                      {(allocationNotification[index].ProcessStatus ===
                        ...
                        "Allocation Extension Requested") && (
                          <div>
                            ...
                            <p>
                              <b>Project Dates:</b>{" "}
                              {new Date(allocationNotification[index].allocStartDate)?.toLocaleDateString('en-GB').replace(/\//g, '-')}{" "}
                              <b>to</b>{" "}
                              {(new Date(allocationNotification[index]?.allocEndDate))?.toLocaleDateString('en-GB').replace(/\//g, '-')}
                            </p>
                            <p>
                              <b>Extended Date:</b>{" "}
                              {new Date(allocationNotification[index].extendedDate)?.toLocaleDateString('en-GB').replace(/\//g, '-')}
                            </p>
                          </div>
                        )}
                      {/* {console.log(allocationNotification[index].ProcessStatus, index)} */}
                ...
                >
                  ...
                  {info && clickedinfoItemIndex === index && (
                    <div style={{ padding: "2%" }}>
                      ...
                      {/* {console.log(deAllocationNotification[index].ProcessStatus, index)} */}
                      {(
                        deAllocationNotification[index].ProcessStatus ===
                        "De-Allocation Rejected") && (
                          <div>
