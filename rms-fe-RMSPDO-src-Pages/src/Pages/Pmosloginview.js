...
import { changecount, changestate } from "../Store/reducers/Statusstore";
import Form from 'react-bootstrap/Form';
import { changePmo, changePmoAdmin } from "../Store/reducers/Statusstore";
import { GiExitDoor } from "react-icons/gi";
import BenchExit from "./BenchExit";
import DashboardNavigation from "./DashboardNavigation";
import "../CSS/DashBoardNavigation.css"
import { ImMenu } from "react-icons/im";
import statistics from './../Pages/LoginPage/Images/statisctics.png'
import {
  Dropdown,
  DropdownToggle,
  DropdownMenu,
...
function Pmosloginview(props) {
  ...
  const [benchExit, setbenchExit] = useState()
        ...
        <Col xs={4}>
          <div style={{ display: "flex" }}>
            ...
            >
              ...
              <DropdownMenu className={show ? 'show1' : ''}  style={{padding:"7% 7px",width:"fit-content"}}>
                <div style={{ display: "flex", padding: '3%', width: "53%" }}>
                 <div> <DropdownItem onClick={() => { navigate('/dashboard') }} >
                    Dashboard
                  </DropdownItem></div>
                  <div><DropdownItem onClick={() => { navigate('/pmosloginview') }} className="bg-info">
                    Workbench
                  </DropdownItem></div>
                </div>
              </DropdownMenu>
           ...
           <div style={{width:'20%'}} onClick={() => { navigate('/dashboard') }}><img src={statistics} width="15%"/></div>
          </div>
        </Col>
       
        <Col xs="6" style={{ marginLeft: "16%" }}>
          <Row>
          <Col>
          <Button
            outline
                ...
                <Timeline
                  ...
                  onItemClick={(itemId) => {
                    ...
                    if (
                      ...
                      setEToast("De-Allocation has already requested");
                    }
                    else if (
                      result2
                    ) {
                      ...
                      {({ getRootProps }) => {
                        return (
                          <div className="timeline" {...getRootProps()}>
                            <div>
                              <h5 style={{ color: "white",marginBottom:"-1px" }}>Resource  Name <sup><Badge>{groups.length}</Badge></sup><span  style={{paddingLeft:"8%"}} onClick={() => setbenchExit(true)}><GiExitDoor fill="white" size="2rem" title="Bench Exit" /></span>
                             </h5>
                            </div>
                          </div>
                        );
